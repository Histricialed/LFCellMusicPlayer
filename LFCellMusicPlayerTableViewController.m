//
//  LFCellMusicPlayerTableViewController.m
//  UITableView
//
...
#import "LFCellMusicPlayerTableViewCell.h"
#import "LJAudioPlayerManager.h"

#define WEAKSELF() __weak __typeof(&*self)weakSelf = self


@interface LFCellMusicPlayerTableViewController()
...
@implementation LFCellMusicPlayerTableViewController
...
- (void)viewDidLoad
{
    ...
    UINib *nib = [UINib nibWithNibName:@"LFCellMusicPlayerTableViewCell" bundle:nil];
    [self.tableView registerNib:nib
         forCellReuseIdentifier:@"LFCellMusicPlayerTableViewCell"];
    for (int i = 0 ; i < 10 ; i++) {
        BNRItem *newItem = [[BNRItemStore sharedStore] createItem];
        NSInteger lastRow = [[[BNRItemStore sharedStore] allItems] indexOfObject:newItem];
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:lastRow inSection:0];
...
- (void)setArrayWithResultDic:(NSDictionary *)dic withCurrentCell:(LFCellMusicPlayerTableViewCell *)currentCell {
    ...
    [LJAudioPlayerManager sharedInstance].AudioPlayerFinishPlayingBlock = ^(BOOL flag, NSIndexPath *indexPath) {
        LFCellMusicPlayerTableViewCell *finishedCell = [self.tableView cellForRowAtIndexPath:indexPath];
        if (flag) {
    ...
    LFCellMusicPlayerTableViewCell *usedPlayedCell = [self.tableView cellForRowAtIndexPath:usedPlayedCellIndexPath];
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[LFCellMusicPlayerTableViewCell class]]) {
        LFCellMusicPlayerTableViewCell *tmpCell = (LFCellMusicPlayerTableViewCell *)cell;
        if ([self.timeArray count] > indexPath.row) {
